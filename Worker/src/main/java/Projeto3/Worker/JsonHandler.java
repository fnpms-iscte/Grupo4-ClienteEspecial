...
public class JsonHandler {
	...
	private void readLectureFile() {
		...
		for (int i = 0; i < this.lecture_file.length(); i++) {
			try {
				...
				while (keys.hasNext()) {
					...
					if (key.contains("Curso")) {
						...
						n_studentsS = lectureJson.getString(key);
							...
							.equals("Turnos com capacidade superior à capacidade das características das salas")) {
				...
				Lecture lecture = new Lecture(i, course, name, shift, class_name, n_students,
						Turnos_com_capacidade_superior, Turno_com_inscrições_superiores, week_day, start_date, end_date,
	...
	private DateTime convertDateTime(String timeS, String dateS) {
		if (!timeS.isEmpty()) {
			...
			if (dateS.isEmpty()) {
				DateTime datetime = new DateTime(0, 12, 31, Integer.parseInt(time[0]), Integer.parseInt(time[1]),
						Integer.parseInt(time[2]));
				return datetime;
			} else {
				String[] date = dateS.split("/");
				DateTime datetime = new DateTime(Integer.parseInt(date[2]), Integer.parseInt(date[1]),
	...
	private void readRoomFile() {
		...
		for (int i = 0; i < room_file.length(); i++) {
			...
			try {
				...
				Room r = new Room(edificio, name, capacidade_normal, capacidade_exame, n_caracteristicas, caract, i);
				rooms.add(r);
			} catch (JSONException e) {
				e.printStackTrace();
			}
